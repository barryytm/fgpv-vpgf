...
function geosearchService($q, $rootScope, stateManager, referenceService, events, debounceService, mapService, geoService, gapiService, appInfo) {
    ...
    function runQuery() {
        ...
        return GSintention.query(`${service.searchValue}*`).then(data => {
    ...
    function getProvinces() {
        return $q(resolve => {
            events.$on(events.rvApiReady, () => {
                GSintention.fetchProvinces().then(val => {
    ...
    function getTypes() {
        return $q(resolve => {
            events.$on(events.rvApiReady, () => {
                GSintention.fetchTypes().then(val => {
                    let typeList = val;
                    resolve(typeList);
                });
            });
